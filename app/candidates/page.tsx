...
import { CANDIDATES, US_STATES, partyColor, type Party } from '@/lib/mock-data'
...
// States that have candidate data
...
export default function CandidatesPage() {
  ...
  const stateName = stateFilter === 'All'
    ? 'All States'
    ...
    <PageShell>
      <div className="flex flex-col min-h-svh bg-background">
        ...
        <header className="bg-card px-5 pt-14 pb-4 border-b border-border">
          ...
          <p className="text-sm text-muted-foreground mt-0.5">{stateName} · {filtered.length} candidate{filtered.length !== 1 ? 's' : ''}</p>
        </header>
